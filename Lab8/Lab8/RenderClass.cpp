...
HRESULT RenderClass::InitBufferShader()
{
    ...
    if (SUCCEEDED(result))
        result = CompileShader(L"NormalPrepass.vs", &m_pNormalPrepassVS, nullptr);
...
void RenderClass::TerminateBufferShader()
{
    ...
    if (m_pNormalPrepassVS)
    {
        m_pNormalPrepassVS->Release();
        m_pNormalPrepassVS = nullptr;
    }

    if (m_pNormalPrepassPS)
    {
        m_pNormalPrepassPS->Release();
...
HRESULT RenderClass::CreateSSAOResources(UINT width, UINT height)
{
    ...
    HRESULT hr = m_pDevice->CreateTexture2D(&normalDesc, nullptr, &m_pNormalTexture);
    if (FAILED(hr))
        return hr;

    hr = m_pDevice->CreateRenderTargetView(m_pNormalTexture, nullptr, &m_pNormalRTV);
    if (FAILED(hr))
        return hr;

    hr = m_pDevice->CreateShaderResourceView(m_pNormalTexture, nullptr, &m_pNormalSRV);
    if (FAILED(hr))
        return hr;
    ...
    hr = m_pDevice->CreateTexture2D(&ssaoDesc, nullptr, &m_pSSAOTexture);
    if (FAILED(hr))
        return hr;
...
void RenderClass::RenderDepthNormalPrepass(const XMMATRIX& viewProj)
{
    ...
    m_pDeviceContext->OMSetRenderTargets(1, &m_pNormalRTV, m_pDepthView);
    m_pDeviceContext->ClearRenderTargetView(m_pNormalRTV, clearNormal);
    m_pDeviceContext->ClearDepthStencilView(m_pDepthView, D3D11_CLEAR_DEPTH, 1.0f, 0);

    m_pDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    m_pDeviceContext->IASetInputLayout(m_pLayout);
    m_pDeviceContext->VSSetShader(m_pNormalPrepassVS, nullptr, 0);
    m_pDeviceContext->PSSetShader(m_pNormalPrepassPS, nullptr, 0);
    m_pDeviceContext->VSSetConstantBuffers(1, 1, &m_pVPBuffer);
    ...
    RenderGroundPlaneDepthNormal();
    RenderAllSceneModelsDepthNormal();

    m_pDeviceContext->RSSetState(nullptr);
    m_pDeviceContext->OMSetRenderTargets(0, nullptr, nullptr);
}

void RenderClass::RenderGroundPlaneDepthNormal()
{
    if (!m_pGroundVB || !m_pGroundIB || m_GroundIndexCount == 0)
        return;
    ...
    ShadowMaterialCB prepassMaterial = {};
    prepassMaterial.AlphaParams = XMFLOAT4(0.0f, 1.0f, 0.5f, 0.0f);
    m_pDeviceContext->UpdateSubresource(m_pShadowMaterialBuffer, 0, nullptr, &prepassMaterial, 0, 0);
...
void RenderClass::RenderAllSceneModelsDepthNormal()
{
    for (const SceneModelInstance& instance : m_SceneModelInstances)
    {
        RenderModelInstanceDepthNormal(instance);
    }
...
void RenderClass::RenderModelInstanceDepthNormal(const SceneModelInstance& instance)
{
...
void RenderClass::RenderGltfNodeDepthNormal(
    ...
    const XMMATRIX& instanceWorld)
{
    ...
    if (node.MeshIndex >= 0 && node.MeshIndex < (int)model.GpuMeshes.size())
    {
        ...
        {
            DrawGltfPrimitiveDepthNormal(model, prim, finalWorld);
        }
    }

    for (int child : node.Children)
    {
        RenderGltfNodeDepthNormal(model, child, finalWorld);
    }
}

void RenderClass::DrawGltfPrimitiveDepthNormal(
    const GltfModelResource& model,
    ...
    const XMMATRIX& world)
{
        ...
        primitive.MaterialIndex < (int)model.Scene.Materials.size())
    {
        ...
        prepassMaterial.AlphaParams.y = mat.BaseColorFactor.w;
        prepassMaterial.AlphaParams.z = mat.AlphaCutoff;
        prepassMaterial.AlphaParams.w = mat.AlphaMask ? 1.0f : 0.0f;
            ...
            model.TextureSRVs[mat.BaseColorTexture] != nullptr)
        {
            ...
            prepassMaterial.AlphaParams.x = 1.0f;
        }
    }
