...
HRESULT RenderClass::Init(HWND hWnd, WCHAR szTitle[], WCHAR szWindowClass[])
{
   ...
   /* if (LoadModelFromFile(L"models/tamioka/Ch_03_01.gltf"))
    ...
    {
        OutputDebugStringA("GLTF load failed\n");
    }

    char buffer[256];
    sprintf_s(buffer, "Meshes: %zu, Materials: %zu, Textures: %zu, Nodes: %zu\n",
        m_GltfScene.Meshes.size(),
        m_GltfScene.Materials.size(),
        m_GltfScene.Textures.size(),
